...
function convertMapToObject(map) {
  ...
  const obj = Object.fromEntries(iter);
  return obj;
}
...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach(([key, value]) =>
    dictionary.has(key) ? rejected++ : (dictionary.set(key, value), added++)
  );
...
function checkPresence(dictionary, keys) {
  ...
  let presence = [];
  keys.forEach((elem) =>
    dictionary.has(elem) ? presence.push(true) : presence.push(false)
  );
  return presence;
}
...
function sortByValues(dictionary) {
  ...
  arr.sort((value1, value2) => value2[1] - value1[1]);
...
function convertSetToDictionary(set) {
  ...
  const map = new Map();
  //set.forEach((elem) => map.set(elem, String.fromCodePoint(elem.at(1)) ));
  for(const value of set){
    map.set(value, value.at(0));
  }
  return map;
}
